...
function queensAttack(n, k, r_q, c_q, obstacles) {
    const count = 0;
    obstacles = obstacles.map(e => [--e[0], --e[1]])
    ...
    // right-top
    let right_top = Math.min(r_q, n-c_q-1);
    ...
    // right-bottom
    let right_bottom = Math.min(n-1-r_q, n-1-c_q);
    ...
    // left-bottom
    let left_bottom = Math.min(n-1-r_q, c_q);
